function [ g ] = createDomainFolded( width , NX , NZsub , NZsupra , intBound , upBound )
...
g.E0T(end-g.NX+1:end,2) = (1 : NX);                                         % added to fold grid
g.E0T(NX : NX : end, 3) = g.E0T((1 : NX : g.numT),4);                       % changed to fold grid
%% Create Mapping: Trapezoid -> chargeEdges
g.CE0T = zeros( g.numT , 4 );
g.CE0T(:,1) = ( 1 : g.numT );
g.CE0T(:,2) = g.CE0T(:,1) + g.numT;
g.CE0T(:,4) = g.CE0T(:,2) + g.numT;
g.CE0T(:,3) = g.CE0T(:,4) + g.numT;
%% Create Mapping: chargeEdges -> Vertices
g.V0CE = zeros ( g.numCE, 2 );
g.V0CE(1:g.numT,1) = g.V0T(1:g.numT,1); 
g.V0CE(1:g.numT,2) = g.V0T(1:g.numT,2); 
g.V0CE(g.numT+1:2*g.numT,1) = g.V0T(1:g.numT,4); 
g.V0CE(g.numT+1:2*g.numT,2) = g.V0T(1:g.numT,3); 
g.V0CE(2*g.numT+1:3*g.numT,1) = g.V0T(1:g.numT,1); 
g.V0CE(2*g.numT+1:3*g.numT,2) = g.V0T(1:g.numT,4); 
g.V0CE(3*g.numT+1:4*g.numT,1) = g.V0T(1:g.numT,2); 
g.V0CE(3*g.numT+1:4*g.numT,2) = g.V0T(1:g.numT,3); 
%% Generate ID of edges
g.idE = zeros( g.numE, 1 );
g.idE(NZsub * NX + 1 : (NZsub + 1) * NX) = -1;
g.idE(1:NX) = 1;
g.idE(g.numE - NZ + 1 : end - NZsupra) = 2;
g.idE(g.numE - NZsupra + 1 : end) = 3;
g.idE(g.numT+1:g.numT+NX,:) = 4;
g.idE(g.numT + NX * (NZsub + 1) + 1 : NX :  g.numT + NX * (NZ + 1) , 1) = 5;
g.idE(g.numT + NX +1 : NX :  g.numT + NX * (NZsub + 1) , 1) = 6;
g.idE0T = g.idE(g.E0T);
...
g.markE0TE0T{1}( sub2ind([g.numT,g.numT], 1:NX , g.numT-NX+1:g.numT) ) = 1;    % added to fold grid
g.markE0TE0T{2}( sub2ind([g.numT,g.numT], 1:g.numT-NX , NX+1:g.numT) ) = 1;
g.markE0TE0T{2}( sub2ind([g.numT,g.numT], g.numT-NX+1:g.numT , 1:NX) ) = 1;    % added to fold grid
...
g.markE0TE0T{3}( sub2ind([g.numT,g.numT], NX:NX:g.numT , 1:NX:g.numT-1) ) = 1; % added to fold grid
...
g.markE0TE0T{4}( sub2ind([g.numT,g.numT], 1:NX:g.numT-1 , NX:NX:g.numT) ) = 1; % added to fold grid
...
%% Create Mapping: Edge -> Trapezoid
g.T0E   = zeros( g.numE , 2 );
g.T0E(1:g.numT,1) = (1 : g.numT);
g.T0E(g.NX+1:g.numT+g.NX,2) = (1:g.numT);
g.T0E(1:g.NX,2) = (g.numT-g.NX+1:g.numT);                                   % added to fold grid
g.T0E(NX * (NZ + 1) + 1 : g.numE - NZ,1) = (1:g.numT);
g.T0E(NX * (NZ + 1) + 1 : g.numE - NZ,2) = (0:g.numT-1);
% g.T0E((NX * (NZ + 1) + 1 + NX:NX:g.numE-NZ),2) = 0;
g.T0E(NX * (NZ + 1) + 1 : NX : g.numE - NZ,2) = (1 : NX : g.numT-1);        % added to fold grid
g.T0E(g.numE - NZ + 1 : end , 2) = (NX : NX : NZ*NX);
% Filling up zeros with correct values
g.T0E(g.T0E(:,1) == 0,1) = g.T0E(g.T0E(:,1) == 0,2);
g.T0E(g.T0E(:,2) == 0,2) = g.T0E(g.T0E(:,2) == 0,1);
end  % function